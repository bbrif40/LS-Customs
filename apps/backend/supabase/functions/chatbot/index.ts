...
An admin will review your request and respond soon. You can check the status of your ticket in the chat or ask me for updates.`,
    ...
    if (!OPENROUTER_API_KEY) {
      // Fallback domain-aware response if no API key is set
      return jsonResponse({
        conversation_id: body.conversation_id || crypto.randomUUID(),
        reply: "Welcome to LS Customs! We offer premium car rentals and on-demand mobile mechanic services across Los Santos. Please browse our vehicle catalog or schedule a mechanic service directly from the menu.",
        context_used: false,
        model: "fallback",
      }, null, 200);
    }
